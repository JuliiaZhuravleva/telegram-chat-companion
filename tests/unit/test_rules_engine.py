...
class TestKeywordTrigger:
    ...
    async def test_contains_match(self) -> None:
        engine = _make_engine([_rule_row(config={
            "keywords": ["hello"],
            "match_type": "contains",
            "action": "notify_admin",
        })])
        actions = await engine.evaluate(
            chat_id=-1001, user_id=100,
...
class TestStickerFlood:
    ...
    async def test_flood_triggered(self) -> None:
        repo = AsyncMock()
        repo.get_active_rules = AsyncMock(return_value=[_rule_row(
...
class TestExecutionModes:
    ...
    async def test_mode_all(self) -> None:
        engine = _make_engine([
            _rule_row(rule_id=1, config={"keywords": ["hello"], "match_type": "contains", "action": "notify_admin"}),
            _rule_row(rule_id=2, config={"keywords": ["hello"], "match_type": "contains", "action": "warn_user", "warning_message": "!"}),
        ])
        actions = await engine.evaluate(
            chat_id=-1001, user_id=100,
            message=_make_message("hello"),
    ...
    async def test_mode_highest_weight(self) -> None:
        engine = _make_engine([
            _rule_row(rule_id=1, weight=1, config={"keywords": ["hello"], "match_type": "contains", "action": "notify_admin"}),
            _rule_row(rule_id=2, weight=10, config={"keywords": ["hello"], "match_type": "contains", "action": "warn_user", "warning_message": "!"}),
        ])
        actions = await engine.evaluate(
            chat_id=-1001, user_id=100,
            message=_make_message("hello"),
...
class TestMandatoryRules:
    ...
    async def test_mandatory_always_fires(self) -> None:
        engine = _make_engine([
            _rule_row(rule_id=1, mandatory=True, weight=1, config={"keywords": ["hello"], "match_type": "contains", "action": "notify_admin"}),
            _rule_row(rule_id=2, mandatory=False, weight=10, config={"keywords": ["hello"], "match_type": "contains", "action": "warn_user", "warning_message": "!"}),
        ])
        actions = await engine.evaluate(
            chat_id=-1001, user_id=100,
            message=_make_message("hello"),
...
class TestEdgeCases:
    ...
    async def test_trigger_recorded(self) -> None:
        repo = AsyncMock()
        repo.get_active_rules = AsyncMock(return_value=[_rule_row(config={
            "keywords": ["hello"],
            "match_type": "contains",
            "action": "notify_admin",
        })])
