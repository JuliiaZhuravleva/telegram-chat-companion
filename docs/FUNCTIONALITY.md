...
- **Retrieval** — per request, the pipeline queries `1 - (embedding <=> $query)` cosine similarity (pgvector), filtered by `expires_at IS NULL OR expires_at > NOW()`. Returns the nearest `max_results` (default 5) memories **unfiltered**; `RAGMemoryService` then drops those below `min_similarity` (default 0.7; chat-overrideable to 0.65) before anything reaches the prompt. The floor moved out of SQL in R1 so that sub-floor rows still reach `retrieval_log` with `above_floor: false` — otherwise a blind turn records nothing and the threshold cannot be re-tuned from its own data.
