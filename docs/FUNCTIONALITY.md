...
- **Retrieval** — per request, the pipeline queries `1 - (embedding <=> $query)` cosine similarity (pgvector), filtered by `expires_at IS NULL OR expires_at > NOW()`. Returns the nearest `max_results` (default 5) memories **unfiltered**; `RAGMemoryService` then drops those below `min_similarity` (0.7, from `rag.min_similarity` in `config/default.yml` — there is no per-chat override; `chat_settings` carries only `rag_enabled`) before anything reaches the prompt. The floor moved out of SQL in R1 so that sub-floor rows still reach `retrieval_log` with `above_floor: false` — otherwise a blind turn records nothing and the threshold cannot be re-tuned from its own data.
...
4. Invalid JSON → `"Невалидный JSON. Попробуйте ещё раз."` (FSM stays active, and the reply carries a ✖️ Отмена button). An unknown `action`, or a `set_reaction` emoji outside Telegram's reaction set, is likewise rejected before the rule is created (FSM stays active). Staying active is deliberate — it lets the admin retry — which is exactly why the state needs escapes that do not require valid input: the cancel button, `/cancel`, any slash command, or simply navigating back to a rules screen (TD-049).
...
| S-11 ✅ FIXED | FSM handler lacks `IsAdmin` filter (TOCTOU) | [rules.py:557](src/bot/handlers/rules.py#L557) | `handle_rule_config_input` relied solely on FSM state. If `admin_ids` was revoked between state entry and JSON send, a now-non-admin could still create a rule for any `chat_id` encoded in state. | Fixed in TD-049: the decorator now carries `IsAdmin()` + `F.chat.type == "private"` + slash guards on both `text` and `caption`. Guarded by `tests/unit/test_fsm_escape.py::TestRuleCreationRequiresAuthorityAtTheTime_Of_Writing`. |
