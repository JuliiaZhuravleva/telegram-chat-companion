...
- id: D-3
  ...
  last_update:
    ts: null
    executor: null
    note: 'P2, отложено (решение [Q1]: сначала P1 A+B, затем P2 C+D). Требует реестр настроек per-chat из плана chat-settings-panel — перед стартом D-3 проверить, что соответствующий FieldSpec/registry приземлён (на 2026-08-06 chat-settings-panel выполнен частично: 8/12 done, 4 blocked).'
  result: null
- id: D-4
