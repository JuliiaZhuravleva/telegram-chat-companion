...
```
R0  гигиена запроса (TD-092)      ✅ в проде 2026-08-18 (PR #48, 882e05b)
R1  порог RAG из SQL в сервис     ✅ смержен 2026-08-18 (PR #49, 7a0121f)
R2  пересобрать golden set + переснять линию   ✅ прогнан 2026-08-19 (см. ниже)
    ↓
S4  chat_chunks + индексатор + бэкфилл   ← сделан 2026-08-19 (см. ниже)
S5  гибридный ретривал, shadow → flip
S6  калибровка                            ← теперь есть на чём
```

**R2 оказался не заблокирован — проверено 2026-08-19.** TD-094 утверждал, что
пересборка требует недоступного DSN базы n8n-эпохи. Верна была только половина:
файла кейсов действительно нет, но обе аналитические базы всё это время лежали на
диске остановленными контейнерами — `rag-analysis-n8n` (36 081 сообщение,
2026-01-18 → 2026-08-02) и `rag-analysis-seed` (2 918 записей `chat_memory`,
БД называется `companion`, а не `rag`, как написано в докстринге скрипта).
`docker start` — и R2 из блокера превратился в получасовой прогон.

**Урок для того, как ведётся долг:** запись «заблокировано» была сделана по факту
отсутствия файла и с тех пор переносилась из хендоффа в хендофф как данность.
Она удерживала S5 почти сутки и стоила бы больше, если бы контейнеры успели
попасть под `docker prune`. Проверять такие пункты дешевле, чем носить.

**Что сделано, чтобы это не повторилось:** обе базы выгружены в
`internal/dumps/*.dump` (custom format, gitignored — там реальная переписка) и
проверены чтением всех блоков данных (`pg_restore -f /dev/null`, не `--list`:
последний читает только оглавление и объявляет обрезанный архив здоровым).
Процедура пересборки записана в [docs/rag-eval-baseline.md](../rag-eval-baseline.md).

**Результат прогона:** blind rate **0.636 → 0.636**, recall@5 и MRR без движения.
Гигиена запроса не сдвинула линию на этом наборе, потому что из 11 авто-кейсов
адрес к боту несут только 2 — регекс отбирает «вспоминающие» формулировки, а это
почти всегда реплаи, в которых триггерного слова нет. Сдвинулось одно: лучшая
схожесть верхнего кейса упала 0.805 → 0.749 при неизменном ранжировании, потому
что документы `chat_memory` сами сделаны из сырого обмена (`X спросил: "бот, …"`)
— триггер там общий шаблон запроса и документа. Отсюда два следствия для S5/S6,
разобранных в baseline-документе: порог 0.7 теперь меряет сдвинутую шкалу, и
схожести двух хранилищ несравнимы между собой.

**Поправка к «R0 и R1 независимы», записанная по факту.** В плане они значились
независимыми, и по существу это верно — но не в git и не в тестах. После мержа
R0 ветка R1 дала конфликт в `tests/unit/test_text_pipeline.py` (обе добавляли
класс в конец файла), а после разрешения конфликта упал тест: R0 проверял, что
RAG-поиск получает очищенный запрос, обращаясь к `search`, тогда как R1
переименовал точку входа в `search_unfiltered`. Ни один CI этого увидеть не мог
— код, который они образуют вместе, до мержа не существовал нигде. Вывод для
следующей пары слайсов: «независимы» означает «можно писать параллельно», а не
«можно мержить не перепроверив»; результат слияния надо прогонять локально
перед мержем второго.
...
## 5.2 Что приехало в S4 (2026-08-19)

**Порядок сдвинут осознанно: S4 приехал раньше R2.** R2 тогда числился
заблокированным (TD-094), а блокировать на нём S4 не было причины: S4 не читает
индекс и не меняет ни одного измерения. Линия нужна там, где сравнивают выдачу —
то есть в S5. R2 прогнан 2026-08-19, до флипа он больше не блокер (см. §5.1).

Приехало ровно то, что перечислено в роадмапе для S4:

- **Миграция 029, `chat_chunks`.** Естественный ключ `(chat_id, thread_id, msg_from,
  msg_to, part)` с `NULLS NOT DISTINCT` — без этой оговорки `NULL <> NULL`, и для
  каждого не-форумного чата (то есть почти для всех) уникальность молча перестаёт
  работать: индекс растёт на каждом проходе, а выдача возвращает один и тот же разговор
  по пять раз. Проверено на реальном pg16, а не по документации.
- **Чанкер** — сессии по паузе 3 ч, TARGET 1200 / HARD_MAX 2600 / ≤80 сообщений,
  перекрытие 2 сообщения и ≤400 символов, снимаемое через паузу. Параметры из
  соседних архивных проектов, не переизобретались.
- **Индексатор** — фоновая задача раз в 15 мин, гейт `save_messages`, только закрытые
  сессии, watermark выводится из самого индекса (`MAX(msg_to)` по чату).
- **`scripts/backfill_chunks.py`** — тот же индексатор в цикле, для случаев, когда ждать
  расписание незачем. Отдельной реализации бэкфилла нет намеренно: бэкфилл, который
  режет иначе, чем индексатор, даёт индекс, спорящий сам с собой.

**Замеренное, а не предположенное (2026-08-19).** У `gemini-embedding-001` отсутствующий
`task_type` возвращает **побайтово тот же вектор**, что `RETRIEVAL_QUERY`. Отсюда два
следствия: сторона запроса уже неявно просит query-половину и в S5 её менять не надо, а
`chat_memory` — это **и есть** RETRIEVAL_QUERY-индекс, поэтому переключать на нём
task_type нельзя ни при каких условиях (это переписало бы пространство под уже
записанными векторами). На одной реальной паре чанк/вопрос асимметрия дала 0.6713 →
0.6922 при промахе 0.5169 → 0.5242, то есть зазор чуть шире. Одна пара — не eval; это
причина предпочесть, а не измеренный выигрыш.

**Допущение §4.1, опровергнутое замером: сессии не режутся по `thread_id`.**
В §4.1 записано «пауза 3 ч на `(chat_id, thread_id)`» — в предположении, что
`chat_messages.message_thread_id` это топик форума. Замер прода 2026-08-19 говорит
другое:

| чат | сообщений | из них `thread_id IS NULL` | различных thread_id | в среднем на «тред» |
|---|---|---|---|---|
| крупнейший | 33 049 | 23 185 | **3 737** | 2.6 |
| второй | 2 506 | 1 590 | 428 | 2.1 |
| третий | 2 210 | 1 289 | 342 | 2.7 |
| остальные | 110–546 | ≈95% | 1–74 | 1.0–10.5 |

Telegram ставит `message_thread_id` не только на топики, но и на обычные **цепочки
ответов** в супергруппе. Ни один продовый чат не форум. Резать сессии по этой колонке
означало бы раскрошить разговор на двухсообщенческие обрывки и — хуже — **развести реплай
и сообщение, на которое он отвечает, в разные чанки**: реплай несёт id цепочки, исходное
сообщение нет. Поэтому чанки чат-широкие, а колонка `thread_id` в таблице остаётся под
форум-режим, для которого сперва нужен способ отличить форум (`chat.is_forum` не хранится).

Тот же факт бьёт по промпту и **не** чинится этим слайсом: `pipeline.process()` выбирает
форумную ветку истории по `message_thread_id is not None`, то есть на каждом реплае в
супергруппе окно схлопывается с 20 сообщений до ~2 «текущих» плюс 10 «из других топиков».
Заведено как TD-102 (High).

**Дефект, найденный замером уже после выката (починен в тот же вечер), и поправка к его
масштабу.** Выборка сообщений шла `ORDER BY created_at`, а watermark — это `message_id`. Две
разные оси: батч, взятый по времени, может покрывать более широкий диапазон id, чем содержит,
и всё, что попало в этот диапазон, но не в батч, навсегда отсекается условием
`message_id > watermark`. Механизм реален, и починка (идти по оси id) оставлена в силе.

**Первая оценка ущерба — 792 сообщения у крупнейшего чата и 352 у второго — была неверной.**
Она сравнивала watermark, достигнутый за **два** прохода, с отсечкой «первые 2000 по времени»,
то есть считала пропущенным почти всё, что успел забрать второй проход. Честный ответ даёт
симуляция обоих порядков на реальных парах `(message_id, created_at)` всех продовых чатов:

| порядок обхода | сообщений | не забрано никогда |
|---|---|---|
| `ORDER BY created_at` (как было) | 37 715 | **3** |
| `ORDER BY message_id` (как стало) | 37 715 | **0** |

То есть расхождение осей на этих данных **локальное**: 418 инверсий у соседних пар в
крупнейшем чате, каждая на одну-две позиции, а не сдвиг блоками. Локальная инверсия меняет
местами соседей и не выносит сообщение за пределы id-диапазона своего батча. Сплошная проверка
по тексту подтверждает: в индексе нет пропусков (второй чат — 1626 сообщений проверено, 0
отсутствуют; крупнейший — выборка 275, 0 отсутствуют).

Починку это не отменяет: три сообщения — это уже потеря без следа, а гарантия «ни одного»
стоит одной строчки в `ORDER BY`. Но масштаб был завышен в ~380 раз, и запись об этом важнее
самой правки — цифра успела уехать в PR, коммит, реестр и память до того, как её перепроверили.

Границы сессий по-прежнему временные, но зазор меряется от **максимального** виденного момента,
иначе одна устаревшая метка времени разрезала бы живой разговор пополам.

Контроль: возврат к `ORDER BY created_at` роняет `TestNothingIsSkippedAcrossPasses` — 20 из 40
сообщений исчезают из индекса. Фикстура там строит **блочное** расхождение осей (id 400–419
проштампованы на 20 минут позже, чем 420–439) — форма, которой на проде нет, но которую данные
могут принять после любого следующего импорта. Юнит-тестами этот класс не ловится в принципе:
дефект живёт в стыке ORDER BY, LIMIT и watermark, выведенного из записанного.

**Решения, отличающиеся от буквы §4.1, и почему:**

- **Топик в заголовке не пишется.** Названий тем мы не храним, а `тема #4231` — это пять
  цифр шума в каждом чанке форумного чата, ровно тот класс мусора в эмбеддинге, ради
  которого делался R0. Слот в рендерере оставлен под название, когда оно появится.
- **GIN по `senders` не создаётся.** Фильтр `senders @>` появится в S5; на паре тысяч
  строк он и без индекса мгновенный, а неиспользуемый индекс — это стоимость записи без
  измеримой пользы. Заводить его вместе с запросом, который можно померить.
- **Шов по границе батча.** Если одна сессия длиннее `messages_per_pass`, она
  индексируется до конца батча, а не откладывается: иначе индексатор встал бы на ней
  навсегда. Содержимое полное, просто одна граница чанка поставлена батчем, а не паузой.
