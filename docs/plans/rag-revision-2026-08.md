...
**Дефект, найденный замером уже после выката (починен в тот же вечер).** Выборка
сообщений шла `ORDER BY created_at`, а watermark — это `message_id`. Две разные оси: батч,
взятый по времени, покрывает **более широкий диапазон id**, чем содержит, и всё, что попало
в этот диапазон, но не в батч, навсегда отсекается условием `message_id > watermark`.

Замер на проде до починки: первый батч в 2000 сообщений у крупнейшего чата растянулся на
2792 id — то есть **792 сообщения не были бы проиндексированы никогда**, у второго чата 352.
Никакого следа: индекс просто оказался бы без четырнадцатой части разговора. Причина
расхождения осей — импорт n8n-эпохи: 1.8% соседних пар имеют время и id в разном порядке.

Починка: выборка идёт по `message_id` — той же оси, что и watermark. Это заодно и более
верный порядок разговора (Telegram выдаёт id в порядке отправки, а `created_at` настолько
хорош, насколько хорош писавший строку). Границы сессий по-прежнему временные, но зазор
меряется от **максимального** виденного момента, иначе одна устаревшая метка времени
разрезала бы живой разговор пополам.

Контроль: возврат к `ORDER BY created_at` роняет `TestNothingIsSkippedAcrossPasses` — 20 из
40 сообщений исчезают из индекса. Юнит-тестами этот класс не ловится в принципе: дефект живёт
в стыке ORDER BY, LIMIT и watermark, выведенного из записанного.
