...
R2  пересобрать golden set + переснять линию   ⛔ заблокирован (TD-094), пропущен
    ↓
...
R2 — прогон, а не разработка, но он **заблокирован**: `internal/eval/` на машине
владельца отсутствует целиком (не только потерянный `cases_auto_harvest.json`),
и пересборка требует DSN базы n8n-эпохи. См. TD-094.
...
**Порядок сдвинут осознанно: S4 приехал раньше R2.** R2 заблокирован (TD-094), а
блокировать на нём S4 не было причины: S4 не читает индекс и не меняет ни одного
измерения. Линия нужна там, где сравнивают выдачу — то есть в S5. До флипа R2 обязан
быть закрыт; до тех пор чанки просто копятся.
