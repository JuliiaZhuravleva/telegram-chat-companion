...
- **Case file:** regenerated by `scripts/harvest_auto_strata.py` from the same
  ...
  below are a re-measurement of one fixed set, not a new set. That is not only
  an argument from determinism — the old case file is gone, but three of the
  four hit similarities reproduce to the third decimal, and the fourth is
  precisely the addressed question R0 changed. A different case set would not
